    ...
    until foundOne
      ...
      when 0
        ...
        if isLetter(c) then state=1
                    ...
                    elsif c == ?= then state = 11
                    elsif c == ?; then state = 12
                    elsif c == ?% then state = 13
                    else
      ...
      when 2
        if isDigit(c) then state = 2
          ...
          #type = :number
          #foundOne = true
          puts "Expected a real number, Found integer"
      ...
      when 8
        ...
        foundOne = true
      when 10
        if isDigit(c) then  state = 10
        elsif c == ?. then
          puts "Unrecognized Token dot found at line ",line," and column ",column,"\n"
          raise UnrecognizedTokenException # "Unrecognized Token"
        else
          type = :number
          foundOne = true
